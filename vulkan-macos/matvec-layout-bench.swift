...
// Native GGUF q4_0 layout: 18-byte blocks, {half d, 16 bytes qs}, interleaved.
// Same R/U hoisting as the split kernel, but the loads are 16-bit because an
// 18-byte stride cannot be read as uint4.
template <uint R, uint U>
inline void q4_glue_mlp(device const ushort* w, device const float4* x,
                        device float* out, uint nblk, uint row0, uint lane)
{
    ...
    uint b = lane;
    for (; b + (U - 1) * 32 < nblk; b += U * 32) {
        ushort raw[R][U][9];       // 18 bytes = 9 ushorts per block
        for (uint u = 0; u < U; ++u)
            for (uint r = 0; r < R; ++r) {
                ulong base = ((ulong)(row0 + r) * nblk + b + u * 32) * 9;
                for (uint h = 0; h < 9; ++h) raw[r][u][h] = w[base + h];
            }
        for (uint u = 0; u < U; ++u) {
            device const float4* xv = x + (b + u * 32) * 8;
            float4 a[8];
            for (uint i = 0; i < 8; ++i) a[i] = xv[i];
            for (uint r = 0; r < R; ++r) {
                float d = float(as_type<half>(raw[r][u][0]));
                float s = 0.0, m = 0.0;
                for (uint i = 0; i < 4; ++i) {
                    uint qi = (uint)raw[r][u][1 + i*2] | ((uint)raw[r][u][2 + i*2] << 16);
                    float4 lo = float4( qi        & 0xF, (qi >>  8) & 0xF,
                                       (qi >> 16) & 0xF, (qi >> 24) & 0xF);
                    float4 hi = float4((qi >>  4) & 0xF, (qi >> 12) & 0xF,
                                       (qi >> 20) & 0xF, (qi >> 28) & 0xF);
...
kernel void NAME(device const ushort* w  [[buffer(0)]],                       \\
                 ...
                 uint lane [[thread_index_in_simdgroup]])                     \\
{                                                                             \\
    q4_glue_mlp<R, U>(w, x, out, nblk, (tg * 8 + sg) * R, lane);              \\
}
...
let qb = buf(M * nblk * 18), sc = buf(M * nblk * 2), xb = buf(K * 4), ob = buf(M * 4)
...
fputs("cooling 150s before measuring...\n", stderr); Thread.sleep(forTimeInterval: 150)
...
let configs = [("k_r2u4",2,4), ("g_r2u4",2,4), ("k_r2u4",2,4), ("g_r2u4",2,4), ("k_r2u4",2,4), ("g_r2u4",2,4)]
...
for (name, R, U) in configs {
    guard let f = lib.makeFunction(name: name),
        ...
        print(pad((name.hasPrefix("g_") ? "GLUE 18B" : "SPLIT   ") + " R=\(R) U=\(U)", 20) + "  pipeline failed"); continue
    }
    var n = UInt32(nblk)
    ...
    print(pad((name.hasPrefix("g_") ? "GLUE 18B" : "SPLIT   ") + " R=\(R) U=\(U)", 20) + lpad("\(R*U)", 12)
